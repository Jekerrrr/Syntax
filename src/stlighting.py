...
def if_h(Text_widget, color):
    ...
    if_first = 1.0
    while True:
        if_first = Text_widget.search("\yif\y", if_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not if_first:
            break
...
def else_h(Text_widget, color):
    Text_widget.tag_remove("tag_elseh", "1.0", Text_widget.index('end'))
    else_first = "1.0"
    while True:
        else_first = Text_widget.search("\yelse\y", else_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not else_first:
            break
...
def elif_h(Text_widget, color):
    Text_widget.tag_remove("tag_elifh", "1.0", Text_widget.index('end'))
    elif_first = "1.0"
    while True:
        elif_first = Text_widget.search("\yelif\y", elif_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not elif_first:
            break
...
def try_h(Text_widget, color):
    Text_widget.tag_remove("tag_tryh", "1.0", Text_widget.index('end'))
    try_first = "1.0"
    while True:
        try_first = Text_widget.search("\ytry\y", try_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not try_first:
            break
...
def except_h(Text_widget, color):
    Text_widget.tag_remove("tag_excepth", "1.0", Text_widget.index('end'))
    except_first = "1.0"
    while True:
        except_first = Text_widget.search("\yexcept\y", except_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not except_first:
            break
...
def finally_h(Text_widget, color):
    Text_widget.tag_remove("tag_finallyh", "1.0", Text_widget.index('end'))
    finally_first = "1.0"
    while True:
        finally_first = Text_widget.search("\yfinally\y", finally_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
...
def for_h(Text_widget, color):
    Text_widget.tag_remove("tag_forh", "1.0", Text_widget.index('end'))
    for_first = "1.0"
    while True:
        for_first = Text_widget.search("\yfor\y", for_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not for_first:
            break
...
def while_h(Text_widget, color):
    Text_widget.tag_remove("tag_whileh", "1.0", Text_widget.index('end'))
    while_first = "1.0"
    while True:
        while_first = Text_widget.search("\ywhile\y", while_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not while_first:
            break
...
def print_h(Text_widget, color):
    Text_widget.tag_remove("tag_printh", "1.0", Text_widget.index('end'))
    print_first = "1.0"
    while True:
        print_first = Text_widget.search("\yprint\y", print_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not print_first:
            break
...
def yield_h(Text_widget, color):
    Text_widget.tag_remove("tag_yieldh", "1.0", Text_widget.index('end'))
    yield_first = "1.0"
    while True:
        yield_first = Text_widget.search("\yyield\y", yield_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not yield_first:
            break
...
def pass_h(Text_widget, color):
    Text_widget.tag_remove("tag_passh", "1.0", Text_widget.index('end'))
    pass_first = "1.0"
    while True:
        pass_first = Text_widget.search("\ypass\y", pass_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not pass_first:
            break
...
def break_h(Text_widget, color):
    Text_widget.tag_remove("tag_breakh", "1.0", Text_widget.index('end'))
    break_first = "1.0"
    while True:
        break_first = Text_widget.search("\ybreak\y", break_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not break_first:
            break
...
def continue_h(Text_widget, color):
    Text_widget.tag_remove("tag_continueh", "1.0", Text_widget.index('end'))
    continue_first = "1.0"
    while True:
        continue_first = Text_widget.search("\ycontinue\y", continue_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
...
def del_h(Text_widget, color):
    Text_widget.tag_remove("tag_delh", "1.0", Text_widget.index('end'))
    del_first = "1.0"
    while True:
        del_first = Text_widget.search("\ydel\y", del_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not del_first:
            break
...
def assert_h(Text_widget, color):
    Text_widget.tag_remove("tag_asserth", "1.0", Text_widget.index('end'))
    assert_first = "1.0"
    while True:
        assert_first = Text_widget.search("\yassert\y", assert_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not assert_first:
            break
...
def input_h(Text_widget, color):
    Text_widget.tag_remove("tag_inputh", "1.0", Text_widget.index('end'))
    input_first = "1.0"
    while True:
        input_first = Text_widget.search("\yinput\y", input_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not input_first:
            break
...
def import_h(Text_widget, color):
    Text_widget.tag_remove("tag_importh", "1.0", Text_widget.index('end'))
    import_first = "1.0"
    while True:
        import_first = Text_widget.search("\yimport\y", import_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not import_first:
            break
...
def from_h(Text_widget, color):
    Text_widget.tag_remove("tag_fromh", "1.0", Text_widget.index('end'))
    from_first = "1.0"
    while True:
        from_first = Text_widget.search("\yfrom\y", from_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not from_first:
            break
...
def with_h(Text_widget, color):
    Text_widget.tag_remove("tag_withh", "1.0", Text_widget.index('end'))
    with_first = "1.0"
    while True:
        with_first = Text_widget.search("\ywith\y", with_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not with_first:
            break
...
def float_h(Text_widget, color):
    Text_widget.tag_remove("tag_floath", "1.0", Text_widget.index('end'))
    float_first = "1.0"
    while True:
        float_first = Text_widget.search("\yfloat\y", float_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not float_first:
            break
...
def complex_h(Text_widget, color):
    Text_widget.tag_remove("tag_complexh", "1.0", Text_widget.index('end'))
    complex_first = "1.0"
    while True:
        complex_first = Text_widget.search("\ycomplex\y", complex_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
...
def open_h(Text_widget, color):
    Text_widget.tag_remove("tag_openh", "1.0", Text_widget.index('end'))
    open_first = "1.0"
    while True:
        open_first = Text_widget.search("\yopen\y", open_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not open_first:
            break
...
def as_h(Text_widget, color):
    Text_widget.tag_remove("tag_ash", "1.0", Text_widget.index('end'))
    as_first = "1.0"
    while True:
        as_first = Text_widget.search("\yas\y", as_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not as_first:
            break
...
def in_h(Text_widget, color):
    Text_widget.tag_remove("tag_inh", "1.0", Text_widget.index('end'))
    in_first = "1.0"
    while True:
        in_first = Text_widget.search("\yin\y", in_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not in_first:
            break
...
def or_h(Text_widget, color):
    Text_widget.tag_remove("tag_orh", "1.0", Text_widget.index('end'))
    or_first = "1.0"
    while True:
        or_first = Text_widget.search("\yor\y", or_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not or_first:
            break
...
def and_h(Text_widget, color):
    Text_widget.tag_remove("tag_andh", "1.0", Text_widget.index('end'))
    and_first = "1.0"
    while True:
        and_first = Text_widget.search("\yand\y", and_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not and_first:
            break
...
def def_h(Text_widget, color):
    Text_widget.tag_remove("tag_defh", "1.0", Text_widget.index('end'))
    def_first = "1.0"
    while True:
        def_first = Text_widget.search("\ydef\y", def_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not def_first:
            break
...
def class_h(Text_widget, color):
    Text_widget.tag_remove("tag_classh", "1.0", Text_widget.index('end'))
    class_first = "1.0"
    while True:
        class_first = Text_widget.search("\yclass\y", class_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not class_first:
            break
...
def str_h(Text_widget, color):
    Text_widget.tag_remove("tag_strh", "1.0", Text_widget.index('end'))
    str_first = "1.0"
    while True:
        str_first = Text_widget.search("\ystr\y", str_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not str_first:
            break
...
def int_h(Text_widget, color):
    Text_widget.tag_remove("tag_inth", "1.0", Text_widget.index('end'))
    int_first = "1.0"
    while True:
        int_first = Text_widget.search("\yint\y", int_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not int_first:
            break
...
def list_h(Text_widget, color):
    Text_widget.tag_remove("tag_listh", "1.0", Text_widget.index('end'))
    list_first = "1.0"
    while True:
        list_first = Text_widget.search("\ylist\y", list_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not list_first:
            break
...
def tuple_h(Text_widget, color):
    Text_widget.tag_remove("tag_tupleh", "1.0", Text_widget.index('end'))
    tuple_first = "1.0"
    while True:
        tuple_first = Text_widget.search("\ytuple\y", tuple_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not tuple_first:
            break
...
def set_h(Text_widget, color):
    Text_widget.tag_remove("tag_seth", "1.0", Text_widget.index('end'))
    set_first = "1.0"
    while True:
        set_first = Text_widget.search("\yset\y", set_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not set_first:
            break
...
def dict_h(Text_widget, color):
    Text_widget.tag_remove("tag_dicth", "1.0", Text_widget.index('end'))
    dict_first = "1.0"
    while True:
        dict_first = Text_widget.search("\ydict\y", dict_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not dict_first:
            break
...
def lambda_h(Text_widget, color):
    Text_widget.tag_remove("tag_lambdah", "1.0", Text_widget.index('end'))
    lambda_first = "1.0"
    while True:
        lambda_first = Text_widget.search("\ylambda\y", lambda_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not lambda_first:
            break
...
def True_h(Text_widget, color):
    Text_widget.tag_remove("tag_Trueh", "1.0", Text_widget.index('end'))
    True_first = "1.0"
    while True:
        True_first = Text_widget.search("\yTrue\y", True_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not True_first:
            break
...
def False_h(Text_widget, color):
    Text_widget.tag_remove("tag_Falseh", "1.0", Text_widget.index('end'))
    False_first = "1.0"
    while True:
        False_first = Text_widget.search("\yFalse\y", False_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not False_first:
            break
...
def None_h(Text_widget, color):
    Text_widget.tag_remove("tag_Noneh", "1.0", Text_widget.index('end'))
    None_first = "1.0"
    while True:
        None_first = Text_widget.search("\yNone\y", None_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not None_first:
            break
...
def nonlocal_h(Text_widget, color):
    Text_widget.tag_remove("tag_nonlocalh", "1.0", Text_widget.index('end'))
    nonlocal_first = "1.0"
    while True:
        nonlocal_first = Text_widget.search("\ynonlocal\y", nonlocal_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
...
def raise_h(Text_widget, color):
    Text_widget.tag_remove("tag_raiseh", "1.0", Text_widget.index('end'))
    raise_first = "1.0"
    while True:
        raise_first = Text_widget.search("\yraise\y", raise_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not raise_first:
            break
...
def return_h(Text_widget, color):
    Text_widget.tag_remove("tag_returnh", "1.0", Text_widget.index('end'))
    return_first = "1.0"
    while True:
        return_first = Text_widget.search("\yreturn\y", return_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not return_first:
            break
...
def global_h(Text_widget, color):
    Text_widget.tag_remove("tag_globalh", "1.0", Text_widget.index('end'))
    global_first = "1.0"
    while True:
        global_first = Text_widget.search("\yglobal\y", global_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not global_first:
            break
...
def Exception_h(Text_widget, color):
    Text_widget.tag_remove("tag_Exceptionh", "1.0", Text_widget.index('end'))
    Exception_first = "1.0"
    while True:
        Exception_first = Text_widget.search("\yException\y", Exception_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
...
def single_qouts_h(Text_widget, color):
    f_get = Text_widget.get(1.0, Text_widget.index('end'))
    y2 = re.findall(r"'(.*?)'", f_get)
    bg = -1
    Text_widget.tag_remove("tag_qs", "1.0", Text_widget.index('end'))
    for i in y2:
        bg += 1
        qs_first = "1.0"
        while True:
            qs_first = Text_widget.search(f"'{y2[bg]}'", qs_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
            if not qs_first:
                break
...
def double_qouts_h(Text_widget, color):
    ...
    if True:
        try:
            ...
            br = -1
            Text_widget.tag_remove("tag_qt", "1.0", Text_widget.index('end'))
            for i in y:
                br += 1
                qt_first = "1.0"
                while True:
                    qt_first = Text_widget.search(f'"{y[br]}"', qt_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
                    if not qt_first:
                        break
...
def function_h(Text_widget, color):
    fine = Text_widget.get(1.0, Text_widget.index('end'))
    y3 = re.findall(r'\.\w+', fine)
    bgg = -1
    Text_widget.tag_remove("tag_fnny", "1.0", Text_widget.index('end'))
    for i in y3:
        bgg += 1
        fnn_first = "1.0"
        while True:
            fnn_first = Text_widget.search(f'{y3[bgg]}', fnn_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
            if not fnn_first:
                break
...
def colon_h(Text_widget, color):
    Text_widget.tag_remove("tag_colonh", "1.0", Text_widget.index('end'))
    colon_first = "1.0"
    while True:
        colon_first = Text_widget.search(":", colon_first, nocase=False, stopindex=Text_widget.index('end'))
        if not colon_first:
            break
...
def plus_h(Text_widget, color):
    Text_widget.tag_remove("tag_plush", "1.0", Text_widget.index('end'))
    plus_first = "1.0"
    while True:
        plus_first = Text_widget.search("+", plus_first, nocase=False, stopindex=Text_widget.index('end'))
        if not plus_first:
            break
...
def minus_h(Text_widget, color):
    Text_widget.tag_remove("tag_minush", "1.0", Text_widget.index('end'))
    minus_first = "1.0"
    while True:
        minus_first = Text_widget.search("-", minus_first, nocase=False, stopindex=Text_widget.index('end'))
        if not minus_first:
            break
...
def star_h(Text_widget, color):
    Text_widget.tag_remove("tag_multiplyh", "1.0", Text_widget.index('end'))
    multiply_first = "1.0"
    while True:
        multiply_first = Text_widget.search("*", multiply_first, nocase=False, stopindex=Text_widget.index('end'))
...
def divide_h(Text_widget, color):
    Text_widget.tag_remove("tag_dividehy", "1.0", Text_widget.index('end'))
    divide_first = "1.0"
    while True:
        divide_first = Text_widget.search("/", divide_first, nocase=False, stopindex=Text_widget.index('end'))
        if not divide_first:
            break
...
def one_h(Text_widget, color):
    Text_widget.tag_remove("tag_oneh", "1.0", Text_widget.index('end'))
    one_first = "1.0"
    while True:
        one_first = Text_widget.search("1", one_first, nocase=False, stopindex=Text_widget.index('end'))
        if not one_first:
            break
...
def two_h(Text_widget, color):
    Text_widget.tag_remove("tag_twoh", "1.0", Text_widget.index('end'))
    two_first = "1.0"
    while True:
        two_first = Text_widget.search("2", two_first, nocase=False, stopindex=Text_widget.index('end'))
        if not two_first:
            break
...
def three_h(Text_widget, color):
    Text_widget.tag_remove("tag_threeh", "1.0", Text_widget.index('end'))
    three_first = "1.0"
    while True:
        three_first = Text_widget.search("3", three_first, nocase=False, stopindex=Text_widget.index('end'))
        if not three_first:
            break
...
def four_h(Text_widget, color):
    Text_widget.tag_remove("tag_fourh", "1.0", Text_widget.index('end'))
    four_first = "1.0"
    while True:
        four_first = Text_widget.search("4", four_first, nocase=False, stopindex=Text_widget.index('end'))
        if not four_first:
            break
...
def five_h(Text_widget, color):
    Text_widget.tag_remove("tag_fiveh", "1.0", Text_widget.index('end'))
    five_first = "1.0"
    while True:
        five_first = Text_widget.search("5", five_first, nocase=False, stopindex=Text_widget.index('end'))
        if not five_first:
            break
...
def six_h(Text_widget, color):
    Text_widget.tag_remove("tag_sixh", "1.0", Text_widget.index('end'))
    six_first = "1.0"
    while True:
        six_first = Text_widget.search("6", six_first, nocase=False, stopindex=Text_widget.index('end'))
        if not six_first:
            break
...
def seven_h(Text_widget, color):
    Text_widget.tag_remove("tag_sevenh", "1.0", Text_widget.index('end'))
    seven_first = "1.0"
    while True:
        seven_first = Text_widget.search("7", seven_first, nocase=False, stopindex=Text_widget.index('end'))
        if not seven_first:
            break
...
def eight_h(Text_widget, color):
    Text_widget.tag_remove("tag_eighth", "1.0", Text_widget.index('end'))
    eight_first = "1.0"
    while True:
        eight_first = Text_widget.search("8", eight_first, nocase=False, stopindex=Text_widget.index('end'))
        if not eight_first:
            break
...
def nine_h(Text_widget, color):
    Text_widget.tag_remove("tag_nineh", "1.0", Text_widget.index('end'))
    nine_first = "1.0"
    while True:
        nine_first = Text_widget.search("9", nine_first, nocase=False, stopindex=Text_widget.index('end'))
        if not nine_first:
            break
...
def zero_h(Text_widget, color):
    Text_widget.tag_remove("tag_zeroh", "1.0", Text_widget.index('end'))
    zero_first = "1.0"
    while True:
        zero_first = Text_widget.search("0", zero_first, nocase=False, stopindex=Text_widget.index('end'))
        if not zero_first:
            break
...
def greater_than_h(Text_widget, color):
    Text_widget.tag_remove("tag_moreh", "1.0", Text_widget.index('end'))
    more_first = "1.0"
    while True:
        more_first = Text_widget.search(">", more_first, nocase=False, stopindex=Text_widget.index('end'))
        if not more_first:
            break
...
def less_than_h(Text_widget, color):
    Text_widget.tag_remove("tag_lessh", "1.0", Text_widget.index('end'))
    less_first = "1.0"
    while True:
        less_first = Text_widget.search("<", less_first, nocase=False, stopindex=Text_widget.index('end'))
        if not less_first:
            break
...
def bar_h(Text_widget, color):
    Text_widget.tag_remove("tag_barh", "1.0", Text_widget.index('end'))
    bar_first = "1.0"
    while True:
        bar_first = Text_widget.search("|", bar_first, nocase=False, stopindex=Text_widget.index('end'))
        if not bar_first:
            break
...
def forward_slash_h(Text_widget, color):
    Text_widget.tag_remove("tag_foreh", "1.0", Text_widget.index('end'))
    fore_first = "1.0"
    while True:
        fore_first = Text_widget.search("\\", fore_first, nocase=False, stopindex=Text_widget.index('end'))
        if not fore_first:
            break
...
def exclamation_h(Text_widget, color):
    Text_widget.tag_remove("tag_exclamationh", "1.0", Text_widget.index('end'))
    exclamation_first = "1.0"
    while True:
        exclamation_first = Text_widget.search("!", exclamation_first, nocase=False, stopindex=Text_widget.index('end'))
...
def square_h(Text_widget, color):
    Text_widget.tag_remove("tag_astringh", "1.0", Text_widget.index('end'))
    astring_first = "1.0"
    while True:
        astring_first = Text_widget.search("^", astring_first, nocase=False, stopindex=Text_widget.index('end'))
...
def fstring_h(Text_widget, color):
    Text_widget.tag_remove("tag_fstringh", "1.0", Text_widget.index('end'))
    fstring_first = "1.0"
    while True:
        fstring_first = Text_widget.search("f'", fstring_first, nocase=False, stopindex=Text_widget.index('end'))
...
def rstring_h(Text_widget, color):
    Text_widget.tag_remove("tag_rstringh", "1.0", Text_widget.index('end'))
    rstring_first = "1.0"
    while True:
        rstring_first = Text_widget.search("r'", rstring_first, nocase=False, stopindex=Text_widget.index('end'))
...
def percentage_h(Text_widget, color):
    Text_widget.tag_remove("tag_percentageh", "1.0", Text_widget.index('end'))
    percentage_first = "1.0"
    while True:
        percentage_first = Text_widget.search("%", percentage_first, nocase=False, stopindex=Text_widget.index('end'))
...
def custom_h(Text_widget, characters, color):
    ...
    Text_widget.tag_remove(tagey, "1.0", Text_widget.index('end'))
    ratata_first = "1.0"
    while True:
        ratata_first = Text_widget.search(f"\y{characters}\y", ratata_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
        if not ratata_first:
            break
...
def custom_chars_h(Text_widget, char, color):
    ...
    ratata_first = "1.0"
    while True:
        ratata_first = Text_widget.search(f"{char}", ratata_first, nocase=False, stopindex=Text_widget.index('end'))
        if not ratata_first:
            break
...
def custom_regex_h(Text_widget, pattern, color):
    ...
    for i in y3:
        ...
        custom_first = "1.0"
        while True:
            custom_first = Text_widget.search(f'{y3[bgh]}', custom_first, nocase=False, stopindex=Text_widget.index('end'))
            if not custom_first:
                break
...
def each_h(Text_widget, color):
    f_gety = Text_widget.get(1.0, Text_widget.index('end'))
    yyer = re.findall(r'\@\w+\b', f_gety)
    bg = -1
    Text_widget.tag_remove("finerrdr", "1.0", Text_widget.index('end'))
    for i in yyer:
        bg += 1
        qs_first = "1.0"
        while True:
            qs_first = Text_widget.search(f"'{yyer[bg]}'", qs_first, nocase=False, stopindex=Text_widget.index('end'), regexp=True)
            if not qs_first:
                break
